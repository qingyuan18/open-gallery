#!/bin/bash
# Build a new AMI based on EKS AL2023 NVIDIA, pre-pulling diffusers-upscale:latest
# (which has the SD x4 Upscaler weights baked in) into containerd's k8s.io
# namespace. Mirrors build-flux-image-baked-ami.sh.
#
# Source AMI: ami-06c3bbfab4ca9568d  (EKS AL2023 NVIDIA 1.31, 80GiB)
# Output:     ami-XXXX                (root sized for ~7-10GiB image cache)
#
# Usage:
#   ./build-diffusers-upscale-ami.sh
...
ROOT_VOLUME_GIB="${ROOT_VOLUME_GIB:-90}"
ROOT_VOLUME_IOPS="${ROOT_VOLUME_IOPS:-6000}"
ROOT_VOLUME_THROUGHPUT_MBPS="${ROOT_VOLUME_THROUGHPUT_MBPS:-1000}"
IMAGE_REPO="${IMAGE_REPO:-diffusers-upscale}"
IMAGE_TAG="${IMAGE_TAG:-latest}"
NEW_AMI_NAME="${NEW_AMI_NAME:-diffusers-upscale-image-baked-al2023-nvidia-1.31-$(date +%Y%m%d-%H%M)}"
...
while [ $SECONDS -lt $DEADLINE ]; do
  CMD_ID=$(aws ssm send-command \
    ...
    --parameters "commands=[\"if [ -f /opt/.image-pulled ]; then ctr -n k8s.io images ls | grep -F '$IMAGE_URI' | head -1; echo IMAGE_READY_OK; else echo IMAGE_NOT_READY_YET; tail -10 /var/log/cloud-init-output.log 2>/dev/null | sed 's/^/  /'; fi\"]" \
...
NEW_AMI_ID=$(aws ec2 create-image \
  ...
  --name "$NEW_AMI_NAME" \
  --description "EKS AL2023 NVIDIA + diffusers-upscale:$IMAGE_TAG (SD x4 Upscaler baked in)" \
  --tag-specifications "ResourceType=image,Tags=[{Key=purpose,Value=karpenter-diffusers-upscale-image-baked}]" \
  --query 'ImageId' --output text)
echo "[build-ami] Creating AMI: $NEW_AMI_ID"

aws ec2 wait image-available --region "$AWS_REGION" --image-ids "$NEW_AMI_ID" || \
  echo "[build-ami] WARN: image-available wait timed out; AMI may still be pending — re-check manually"
...
Next steps:
  1. Update deploy/k8s-manifests/karpenter-ec2nodeclass-diffusers-upscale.yaml
     amiSelectorTerms[0].id  -> $NEW_AMI_ID
  2. kubectl apply -f deploy/k8s-manifests/karpenter-ec2nodeclass-diffusers-upscale.yaml
                   -f deploy/k8s-manifests/karpenter-nodepool-diffusers-upscale.yaml
                   -f deploy/k8s-manifests/diffusers-upscale-deployment.yaml
                   -f deploy/k8s-manifests/diffusers-upscale-service.yaml
                   -f deploy/k8s-manifests/diffusers-upscale-keda-scaledobject.yaml
EOF
